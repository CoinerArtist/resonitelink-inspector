<script lang="ts">
    import type {Field, FieldGeneric, number3x3} from "@coin/resonitelink-ts"
    const { data, changeField }: { data: FieldGeneric<string, number3x3>, changeField: (data: Field) => void } = $props()

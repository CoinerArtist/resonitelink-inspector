<script lang="ts">
    import type {Field, FieldGeneric, number4} from "@coin/resonitelink-ts"
    const { data, changeField }: { data: FieldGeneric<string, number4>, changeField: (data: Field) => void } = $props()
